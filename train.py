...
def train(model, args, epochs=10, experiment_name="DeepLab", lr=0.0001, root=".", device=None,
    ...
    ###IMELE#####
    #train_loader = loaddata.getTrainingData(args.bs,args.csv_train)
    #test_loader= loaddata.getTrainingData(args.bs,args.csv_valid)
    ###3rd####
    train_dataset = Dataset(sub_dir=args.train_data, args=args)
    val_dataset = Dataset(sub_dir=args.valid_data, args=args)
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            # print("depth gt shape: ",depth.shape)
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Validation") if is_rank_zero(
            ...
            if args.garg_crop or args.eigen_crop:
                ...
                gt_height=gt_depth.shape[1]
                gt_width=gt_depth.shape[2]
